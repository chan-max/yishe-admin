...
const scheduleRefreshMyClients = (delay = 120) => {
  if (websocketClient.state.status !== 'connected') {
    return
  }
...
const handleRuntimeConnectionChanged = (event: RuntimeConnectionChangedEvent) => {
  ...
  const clientSource =
    ...
    (typeof connection?.query?.clientSource === 'string' ? connection.query.clientSource : undefined) ||
    (typeof connection?.clientInfo?.source === 'string' ? connection.clientInfo.source : undefined) ||
    ''
  ...
  replaceMyClient(clientId, (previous) => ({
    ...
    lastOnlineAt:
      event.action === 'removed'
        ...
        : connection.lastOnlineAt || connection.connectedAt || event.reportedAt || previous?.lastOnlineAt || null,
    lastOfflineAt:
      event.action === 'removed'
...
const watchWebSocketStatus = () => {
  ...
  if (websocketClient.state.status === 'connected') {
    ...
    stopClientConnectionCheck({
      clearClients:
        websocketClient.state.status === 'idle' ||
        websocketClient.state.status === 'disconnected',
    })
  }
}
