<template>
  <div>
          ...
          <el-dropdown trigger="hover">
            ...
            <template #dropdown>
              <el-dropdown-menu>
                ...
                <el-dropdown-item @click="openImageOption(row)"
                  >编辑图片模板信息</el-dropdown-item
                >
    ...
    <!-- 编辑图片模板信息 -->
    <el-dialog v-model="imageOptionDialogVisible" title="编辑图片模板信息" fullscreen>
          ...
          >
            ...
            <div class="details">
              <div class="meta">
                ...
                <div class="line">
                  比例：<span>{{ imageMetaMap[url]?.ratio || "-" }}</span>
                </div>
              </div>
                ...
                <div class="mode-info">
                  ...
                  <div class="mode-description">
                    <div class="mode-brief">
                      <span class="mode-desc-text">{{
                        getModeDescription(
                          manualConfigs[index]?.mode || "topLeftAutoHeight"
                        ).description
                      }}</span>
                      <!-- 固定尺寸模式提示 -->
                      <el-alert
                      ...
                      >
                        <template #title>
                          <span style="font-size: 12px"
                            >由于尺寸固定，素材图会自动裁剪为指定宽高，不保持原始宽高比</span
                          >
                        </template>
                      </el-alert>
                      <el-popover
                      ...
                      >
                        ...
                        <template #default>
                          <div
                            class="mode-details-popover"
                            v-if="
                              manualConfigs[index].mode === 'topLeftAutoHeight' ||
                              !manualConfigs[index]?.mode
                            "
                          >
                            <div class="detail-section">
                              ...
                              <ul class="detail-list">
                                <li>
                                  图片的<strong>左上角</strong>会与您指定的位置坐标对齐
                                </li>
                                <li>
                                  位置坐标使用百分比表示：(0, 0) 表示模板左上角，(100,
                                  100) 表示模板右下角
                                </li>
                                <li>
                                  宽度由
                                  <code>widthPercent</code>
                                  参数控制，表示占模板宽度的百分比
                                </li>
                                <li>
                                  高度会根据图片的原始宽高比自动计算，保持图片不变形
                                </li>
                              </ul>
                            </div>
                            <div class="detail-section">
                            ...
                            <div class="detail-section">
                              ...
                              <div class="example-box">
                                <div class="example-item">
                                  ...
                                  <code
                                    >{ position: { xPercent: 0, yPercent: 0 }, size: {
                                    widthPercent: 30 } }</code
                                  >
                                  <div class="example-desc">
                                    图片左上角对齐模板左上角，宽度为模板的30%
                                  </div>
                                </div>
                                <div class="example-item">
                                  <strong>示例2：居中偏上</strong>
                                  <code
                                    >{ position: { xPercent: 35, yPercent: 10 }, size: {
                                    widthPercent: 50 } }</code
                                  >
                                  <div class="example-desc">
                                    图片左上角位于模板中心偏上位置，宽度为模板的50%
                                  </div>
                                </div>
                                <div class="example-item">
                                  <strong>示例3：右下角区域</strong>
                                  <code
                                    >{ position: { xPercent: 70, yPercent: 70 }, size: {
                                    widthPercent: 25 } }</code
                                  >
                                  <div class="example-desc">
                                    图片左上角位于右下角区域，宽度为模板的25%
                                  </div>
                                </div>
                              </div>
                            </div>
                          ...
                          <div
                            class="mode-details-popover"
                            v-else-if="manualConfigs[index].mode === 'topLeftFixedSize'"
                          >
                            <div class="detail-section">
                              <div class="detail-title">
                                <el-icon><InfoFilled /></el-icon>
                              ...
                              <ul class="detail-list">
                                <li>
                                  图片的<strong>左上角</strong>会与您指定的位置坐标对齐（与topLeftAutoHeight模式相同）
                                </li>
                                <li>
                                  位置坐标使用百分比表示：(0, 0) 表示模板左上角，(100,
                                  100) 表示模板右下角
                                ...
                                <li>
                                  高度由
                                  <code>heightPercent</code>
                                  参数控制，表示占模板高度的百分比
                                </li>
                                <li>
                                  <strong>重要：</strong
                                  >素材图会被裁剪为指定的宽高尺寸，不保持原始宽高比
                                </li>
                                <li>
                                  裁剪方式：从素材图的左上角开始，按照指定尺寸进行裁剪
                                </li>
                              </ul>
                            </div>
                            <div class="detail-section">
                            ...
                            <div class="detail-section">
                              ...
                              <div class="example-box">
                                <div class="example-item">
                                  ...
                                  <code
                                    >{ mode: "topLeftFixedSize", position: { xPercent: 10,
                                    yPercent: 10 }, size: { widthPercent: 30,
                                    heightPercent: 30 } }</code
                                  >
                                  <div class="example-desc">
                                    素材被裁剪为30%×30%的正方形，放置在左上角区域
                                  </div>
                                </div>
                                <div class="example-item">
                                  <strong>示例2：固定矩形区域</strong>
                                  <code
                                    >{ mode: "topLeftFixedSize", position: { xPercent: 50,
                                    yPercent: 20 }, size: { widthPercent: 40,
                                    heightPercent: 20 } }</code
                                  >
                                  <div class="example-desc">
                                    素材被裁剪为40%×20%的矩形，放置在中心偏上位置
                                  </div>
                                </div>
                              </div>
                            </div>
      ...
      <template #footer>
        ...
        <el-button type="primary" @click="saveImageOptions" :loading="imageOptionSaving"
          >保存</el-button
        >
      </template>
    </el-dialog>

    <!-- 图片预览组件已通过 createImageViewer 动态创建 -->
  </div>
</template>
...
import {
  pageTemplateGroup2D,
  createTemplateGroup2D,
  updateTemplateGroup2D,
  deleteTemplateGroup2D,
} from "@/api/templateGroup2D";
...
// 计算图片容器的样式：高度固定，宽度根据比例自适应
function getImageItemStyle(url: string) {
  const meta = imageMetaMap[url];
  if (!meta || !meta.w || !meta.h) {
    // 如果还没有加载元数据，使用默认宽高比（假设是正方形）
    return {
      height: "100px",
      width: "100px",
    };
  }
  // 高度固定为 100px，宽度根据原始宽高比计算
  const fixedHeight = 100;
  const width = (meta.w / meta.h) * fixedHeight;
  return {
...
function openImageOption(row) {
  ...
  manualConfigs.value = urls.map((_, idx) => {
    ...
    try {
      ...
      if (
        config.mode === "topLeftFixedSize" &&
        (config.fitMode === undefined || config.fitMode === null)
      ) {
        config.fitMode = "crop";
      }
      return config;
...
function getOverlayStyle(url: string, index: number) {
  ...
  if (mode === "topLeftFixedSize") {
    // 固定尺寸模式：使用指定的宽高百分比
    const widthPercent =
      typeof sizeCfg.widthPercent === "number" ? sizeCfg.widthPercent : 30;
    const heightPercent =
      typeof sizeCfg.heightPercent === "number" ? sizeCfg.heightPercent : 30;
  ...
  if (mode === "topLeftAutoHeight") {
    // 左上角对齐模式：从左上角渐变到右下角
    background = `linear-gradient(135deg, rgba(64, 158, 255, ${baseAlpha}) 0%, rgba(64, 158, 255, ${
      baseAlpha * 0.5
    }) 50%, rgba(64, 158, 255, ${baseAlpha * 0.1}) 100%)`;
  } else {
    // 固定尺寸模式：纯色背景（跟随透明度）
  ...
  return {
    ...
    transform: `rotate(${
      typeof config.rotationDegrees === "number" ? config.rotationDegrees : 0
    }deg)`,
  };
}
    ...
    for (let idx = 0; idx < imageOptionsDraft.value.length; idx++) {
      ...
      if (txt && txt.trim()) {
        try {
          ...
          ElMessage.error(
            `第 ${idx + 1} 张图片的模板信息不是有效的 JSON/对象：${e?.message || ""}`
          );
          return;
        }
      } else {
...
function getModeDescription(mode: string): { title: string; description: string } {
  ...
  const descriptions: Record<string, { title: string; description: string }> = {
    topLeftAutoHeight: {
      ...
      description:
        "图片的左上角与您指定的位置坐标对齐，宽度由 widthPercent 控制，高度根据原始宽高比自动计算。这是最常用的定位方式，适合精确控制素材在模板中的位置。",
    },
    topLeftFixedSize: {
      title: "左上角对齐-固定尺寸（topLeftFixedSize）",
      description:
        "图片的左上角与您指定的位置坐标对齐，宽度和高度分别由 widthPercent 和 heightPercent 控制。素材图会被裁剪为指定的宽高尺寸，不保持原始宽高比。适合需要精确尺寸控制的场景。",
    },
    center: {
      title: "居中对齐模式（center）",
...
async function getList() {
  loading.value = true;
  try {
    const res = await pageTemplateGroup2D({
...
const rules = {
  ...
  keywords: [{ max: 500, message: "长度不超过500", trigger: "blur" }],
  description: [{ max: 1000, message: "长度不超过1000", trigger: "blur" }],
};
...
.image-option-item .preview .el-image {
  width: 100%;
  height: 100%;
  object-fit: contain;
  display: block;
...
.image-option-item .details {
  display: flex;
  flex-direction: column;
  gap: 8px;
}
.image-option-item .meta {
  font-size: 12px;
  color: var(--el-text-color-secondary);
  display: flex;
  gap: 12px;
}
.image-option-item .config-editor {
  margin-top: 4px;
}
.image-option-body {
