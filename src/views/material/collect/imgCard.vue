<template>
  <span style="height: 200px;" class="img-card relative" ref="imgCardRef" :class="{ imgIsChecked: isChecked }">
    <!-- 使用 el-image 替换原生 img -->

    <el-image style="height: 200px;min-width: 160px;" fit="contain" :src="info.ossObjectName" hide-on-click-modal
      @click="doCheck">
      <!-- 加载中的占位符 -->
    ...
    <transition name="fade">
      <div class="absolute bottom-0 left-0 w-full flex pb-6 flex-wrap gap-2 justify-center img-card-bottom"
        v-if="isHovered">
        <div>
          <el-button size="small" round type="danger" @click="doDelete">删除</el-button>
        </div>
        <div>
          <el-button size="small" round type="primary" @click="doPreview">预览</el-button>
        </div>
        <div>
          <el-button size="small" round type="primary" @click="$emit('checkType', info, 1)"> 入库</el-button>
        </div>
      </div>
    </transition>
  </span>
...
const emits = defineEmits(['checkType', 'delete', 'preview', 'check']);
