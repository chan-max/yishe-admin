<template>
  <div>
    <div class="pb-4 flex flex-wrap justify-start gap-4 items-center search-bar">
      ...
      <el-button type="danger" :disabled="!ids.length" @click="handleDelete(null)">
        批量删除
      </el-button>
    </div>
  ...
  try {
    ...
    if (resultData) {
      ...
      if (dialogVisible.value && form.value.id === aiGenRow.id) {
        form.value.name = resultData.name;
        form.value.description = resultData.description;
        form.value.keywords = resultData.keywords;
      }
      // 也可以直接更新表格行
      aiGenRow.name = resultData.name;
      aiGenRow.description = resultData.description;
      aiGenRow.keywords = resultData.keywords;
    }
...
const rules = {
  name: [{ max: 1000, message: "模型名称长度不能超过 1000 个字符", trigger: "blur" }],
