...
export interface TheguardianItem { id: string; title: string; url: string; [key: string]: any; }
export interface TheguardianResult { success: boolean; query: string; count: number; total: number; items: TheguardianItem[]; error?: string; }
export interface TheguardianServiceStatus { connected?: boolean; available?: boolean; status?: string; state?: string; message?: string; supportedCommands?: string[]; }
export interface TheguardianClientVO { clientId: string; isOnline?: boolean; [mod]?: TheguardianServiceStatus | null; }
export interface TheguardianCommandResponse { success: boolean; message: string; data?: any; }

function sendCmd(clientId: string, name: string, payload?: Record<string, any>) {
  return sendServiceCommand({ target: { clientId, pluginKey: 'theguardian' }, command: { name, payload: payload || {} }, mode: 'production' }) as Promise<TheguardianCommandResponse>
}
...
export function searchAndWait(clientId: string, payload: any): Promise<TheguardianResult> {
  return (async () => {
    ...
    return (result.data?.data || result.result || {}) as TheguardianResult;
  })();
}
