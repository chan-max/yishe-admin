/**
 * MIT Tech Review 数据接口封装
 */
import { sendServiceCommand, type ServiceCommandDTO } from '@/api/system/websocket'
import { websocketClient } from '@/services/websocketClient'

export interface MittechreviewItem { id: string; title: string; url: string; [key: string]: any; }
export interface MittechreviewResult { success: boolean; query: string; count: number; total: number; items: MittechreviewItem[]; error?: string; }
export interface MittechreviewServiceStatus { connected?: boolean; available?: boolean; status?: string; state?: string; message?: string; supportedCommands?: string[]; }
export interface MittechreviewClientVO { clientId: string; isOnline?: boolean; [mod]?: MittechreviewServiceStatus | null; }
export interface MittechreviewCommandResponse { success: boolean; message: string; data?: any; }

function sendCmd(clientId: string, name: string, payload?: Record<string, any>) {
  return sendServiceCommand({ target: { clientId, pluginKey: 'mittechreview' }, command: { name, payload: payload || {} }, mode: 'production' }) as Promise<MittechreviewCommandResponse>
}
...
export function searchAndWait(clientId: string, payload: any): Promise<MittechreviewResult> {
  return (async () => {
    ...
    return (result.data?.data || result.result || {}) as MittechreviewResult;
  })();
}
