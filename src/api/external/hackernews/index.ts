...
export interface HackernewsItem { id: string; title: string; url: string; [key: string]: any; }
export interface HackernewsResult { success: boolean; query: string; count: number; total: number; items: HackernewsItem[]; error?: string; }
export interface HackernewsServiceStatus { connected?: boolean; available?: boolean; status?: string; state?: string; message?: string; supportedCommands?: string[]; }
export interface HackernewsClientVO { clientId: string; isOnline?: boolean; [mod]?: HackernewsServiceStatus | null; }
export interface HackernewsCommandResponse { success: boolean; message: string; data?: any; }

function sendCmd(clientId: string, name: string, payload?: Record<string, any>) {
  return sendServiceCommand({ target: { clientId, pluginKey: 'hackernews' }, command: { name, payload: payload || {} }, mode: 'production' }) as Promise<HackernewsCommandResponse>
}
...
export function searchAndWait(clientId: string, payload: any): Promise<HackernewsResult> {
  return (async () => {
    ...
    return (result.data?.data || result.result || {}) as HackernewsResult;
  })();
}
