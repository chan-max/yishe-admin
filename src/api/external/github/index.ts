...
export interface GithubItem { id: string; title: string; url: string; [key: string]: any; }
export interface GithubResult { success: boolean; query: string; count: number; total: number; items: GithubItem[]; error?: string; }
export interface GithubServiceStatus { connected?: boolean; available?: boolean; status?: string; state?: string; message?: string; supportedCommands?: string[]; }
export interface GithubClientVO { clientId: string; isOnline?: boolean; [mod]?: GithubServiceStatus | null; }
export interface GithubCommandResponse { success: boolean; message: string; data?: any; }

function sendCmd(clientId: string, name: string, payload?: Record<string, any>) {
  return sendServiceCommand({ target: { clientId, pluginKey: 'github' }, command: { name, payload: payload || {} }, mode: 'production' }) as Promise<GithubCommandResponse>
}
...
export function searchAndWait(clientId: string, payload: any): Promise<GithubResult> {
  return (async () => {
    ...
    return (result.data?.data || result.result || {}) as GithubResult;
  })();
}
