...
export interface TechcrunchItem { id: string; title: string; url: string; [key: string]: any; }
export interface TechcrunchResult { success: boolean; query: string; count: number; total: number; items: TechcrunchItem[]; error?: string; }
export interface TechcrunchServiceStatus { connected?: boolean; available?: boolean; status?: string; state?: string; message?: string; supportedCommands?: string[]; }
export interface TechcrunchClientVO { clientId: string; isOnline?: boolean; [mod]?: TechcrunchServiceStatus | null; }
export interface TechcrunchCommandResponse { success: boolean; message: string; data?: any; }

function sendCmd(clientId: string, name: string, payload?: Record<string, any>) {
  return sendServiceCommand({ target: { clientId, pluginKey: 'techcrunch' }, command: { name, payload: payload || {} }, mode: 'production' }) as Promise<TechcrunchCommandResponse>
}
...
export function searchAndWait(clientId: string, payload: any): Promise<TechcrunchResult> {
  return (async () => {
    ...
    return (result.data?.data || result.result || {}) as TechcrunchResult;
  })();
}
