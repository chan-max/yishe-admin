...
export interface ProducthuntItem { id: string; title: string; url: string; [key: string]: any; }
export interface ProducthuntResult { success: boolean; query: string; count: number; total: number; items: ProducthuntItem[]; error?: string; }
export interface ProducthuntServiceStatus { connected?: boolean; available?: boolean; status?: string; state?: string; message?: string; supportedCommands?: string[]; }
export interface ProducthuntClientVO { clientId: string; isOnline?: boolean; [mod]?: ProducthuntServiceStatus | null; }
export interface ProducthuntCommandResponse { success: boolean; message: string; data?: any; }

function sendCmd(clientId: string, name: string, payload?: Record<string, any>) {
  return sendServiceCommand({ target: { clientId, pluginKey: 'producthunt' }, command: { name, payload: payload || {} }, mode: 'production' }) as Promise<ProducthuntCommandResponse>
}
...
export function searchAndWait(clientId: string, payload: any): Promise<ProducthuntResult> {
  return (async () => {
    ...
    return (result.data?.data || result.result || {}) as ProducthuntResult;
  })();
}
