...
export interface ThevergeItem { id: string; title: string; url: string; [key: string]: any; }
export interface ThevergeResult { success: boolean; query: string; count: number; total: number; items: ThevergeItem[]; error?: string; }
export interface ThevergeServiceStatus { connected?: boolean; available?: boolean; status?: string; state?: string; message?: string; supportedCommands?: string[]; }
export interface ThevergeClientVO { clientId: string; isOnline?: boolean; [mod]?: ThevergeServiceStatus | null; }
export interface ThevergeCommandResponse { success: boolean; message: string; data?: any; }

function sendCmd(clientId: string, name: string, payload?: Record<string, any>) {
  return sendServiceCommand({ target: { clientId, pluginKey: 'theverge' }, command: { name, payload: payload || {} }, mode: 'production' }) as Promise<ThevergeCommandResponse>
}
...
export function searchAndWait(clientId: string, payload: any): Promise<ThevergeResult> {
  return (async () => {
    ...
    return (result.data?.data || result.result || {}) as ThevergeResult;
  })();
}
