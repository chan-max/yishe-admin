...
export interface ArxivItem { id: string; title: string; url: string; [key: string]: any; }
export interface ArxivResult { success: boolean; query: string; count: number; total: number; items: ArxivItem[]; error?: string; }
export interface ArxivServiceStatus { connected?: boolean; available?: boolean; status?: string; state?: string; message?: string; supportedCommands?: string[]; }
export interface ArxivClientVO { clientId: string; isOnline?: boolean; [mod]?: ArxivServiceStatus | null; }
export interface ArxivCommandResponse { success: boolean; message: string; data?: any; }

function sendCmd(clientId: string, name: string, payload?: Record<string, any>) {
  return sendServiceCommand({ target: { clientId, pluginKey: 'arxiv' }, command: { name, payload: payload || {} }, mode: 'production' }) as Promise<ArxivCommandResponse>
}
...
export function searchAndWait(clientId: string, payload: any): Promise<ArxivResult> {
  return (async () => {
    ...
    return (result.data?.data || result.result || {}) as ArxivResult;
  })();
}
