...
export interface GdeltItem { id: string; title: string; url: string; [key: string]: any; }
export interface GdeltResult { success: boolean; query: string; count: number; total: number; items: GdeltItem[]; error?: string; }
export interface GdeltServiceStatus { connected?: boolean; available?: boolean; status?: string; state?: string; message?: string; supportedCommands?: string[]; }
export interface GdeltClientVO { clientId: string; isOnline?: boolean; [mod]?: GdeltServiceStatus | null; }
export interface GdeltCommandResponse { success: boolean; message: string; data?: any; }

function sendCmd(clientId: string, name: string, payload?: Record<string, any>) {
  return sendServiceCommand({ target: { clientId, pluginKey: 'gdelt' }, command: { name, payload: payload || {} }, mode: 'production' }) as Promise<GdeltCommandResponse>
}
...
export function searchAndWait(clientId: string, payload: any): Promise<GdeltResult> {
  return (async () => {
    ...
    return (result.data?.data || result.result || {}) as GdeltResult;
  })();
}
