...
export interface ucoingeckoResult { success: boolean; query: string; count: number; total: number; items: any[]; error?: string; }
export interface ucoingeckoServiceStatus { connected?: boolean; available?: boolean; status?: string; state?: string; message?: string; }
export interface ucoingeckoClientVO { clientId: string; isOnline?: boolean; coingecko?: ucoingeckoServiceStatus | null; }

function sendCmd(clientId: string, name: string, payload?: Record<string, any>) {
  return sendServiceCommand({ target: { clientId, pluginKey: 'coingecko' }, command: { name, payload: payload || {} }, mode: 'production' }) as Promise<any>
}
...
export function searchAndWait(clientId: string, payload: any): Promise<ucoingeckoResult> {
  return (async () => {
    ...
    return (result.data?.data || result.result || {}) as ucoingeckoResult;
  })();
}
