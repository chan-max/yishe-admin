...
export interface udictionaryResult { success: boolean; query: string; count: number; total: number; items: any[]; error?: string; }
export interface udictionaryServiceStatus { connected?: boolean; available?: boolean; status?: string; state?: string; message?: string; }
export interface udictionaryClientVO { clientId: string; isOnline?: boolean; dictionary?: udictionaryServiceStatus | null; }

function sendCmd(clientId: string, name: string, payload?: Record<string, any>) {
  return sendServiceCommand({ target: { clientId, pluginKey: 'dictionary' }, command: { name, payload: payload || {} }, mode: 'production' }) as Promise<any>
}
...
export function searchAndWait(clientId: string, payload: any): Promise<udictionaryResult> {
  return (async () => {
    ...
    return (result.data?.data || result.result || {}) as udictionaryResult;
  })();
}
