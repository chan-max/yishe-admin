...
export interface ufrankfurterResult { success: boolean; query: string; count: number; total: number; items: any[]; error?: string; }
export interface ufrankfurterServiceStatus { connected?: boolean; available?: boolean; status?: string; state?: string; message?: string; }
export interface ufrankfurterClientVO { clientId: string; isOnline?: boolean; frankfurter?: ufrankfurterServiceStatus | null; }

function sendCmd(clientId: string, name: string, payload?: Record<string, any>) {
  return sendServiceCommand({ target: { clientId, pluginKey: 'frankfurter' }, command: { name, payload: payload || {} }, mode: 'production' }) as Promise<any>
}
...
export function searchAndWait(clientId: string, payload: any): Promise<ufrankfurterResult> {
  return (async () => {
    ...
    return (result.data?.data || result.result || {}) as ufrankfurterResult;
  })();
}
