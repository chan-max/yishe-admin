...
export interface GovcnItem { id: string; title: string; url: string; [key: string]: any; }
export interface GovcnResult { success: boolean; query: string; count: number; total: number; items: GovcnItem[]; error?: string; }
export interface GovcnServiceStatus { connected?: boolean; available?: boolean; status?: string; state?: string; message?: string; supportedCommands?: string[]; }
export interface GovcnClientVO { clientId: string; isOnline?: boolean; [mod]?: GovcnServiceStatus | null; }
export interface GovcnCommandResponse { success: boolean; message: string; data?: any; }

function sendCmd(clientId: string, name: string, payload?: Record<string, any>) {
  return sendServiceCommand({ target: { clientId, pluginKey: 'govcn' }, command: { name, payload: payload || {} }, mode: 'production' }) as Promise<GovcnCommandResponse>
}
...
export function searchAndWait(clientId: string, payload: any): Promise<GovcnResult> {
  return (async () => {
    ...
    return (result.data?.data || result.result || {}) as GovcnResult;
  })();
}
