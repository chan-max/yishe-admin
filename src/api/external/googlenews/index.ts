...
export interface GooglenewsItem { id: string; title: string; url: string; [key: string]: any; }
export interface GooglenewsResult { success: boolean; query: string; count: number; total: number; items: GooglenewsItem[]; error?: string; }
export interface GooglenewsServiceStatus { connected?: boolean; available?: boolean; status?: string; state?: string; message?: string; supportedCommands?: string[]; }
export interface GooglenewsClientVO { clientId: string; isOnline?: boolean; [mod]?: GooglenewsServiceStatus | null; }
export interface GooglenewsCommandResponse { success: boolean; message: string; data?: any; }

function sendCmd(clientId: string, name: string, payload?: Record<string, any>) {
  return sendServiceCommand({ target: { clientId, pluginKey: 'googlenews' }, command: { name, payload: payload || {} }, mode: 'production' }) as Promise<GooglenewsCommandResponse>
}
...
export function searchAndWait(clientId: string, payload: any): Promise<GooglenewsResult> {
  return (async () => {
    ...
    return (result.data?.data || result.result || {}) as GooglenewsResult;
  })();
}
