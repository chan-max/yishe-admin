...
export interface NprItem { id: string; title: string; url: string; [key: string]: any; }
export interface NprResult { success: boolean; query: string; count: number; total: number; items: NprItem[]; error?: string; }
export interface NprServiceStatus { connected?: boolean; available?: boolean; status?: string; state?: string; message?: string; supportedCommands?: string[]; }
export interface NprClientVO { clientId: string; isOnline?: boolean; [mod]?: NprServiceStatus | null; }
export interface NprCommandResponse { success: boolean; message: string; data?: any; }

function sendCmd(clientId: string, name: string, payload?: Record<string, any>) {
  return sendServiceCommand({ target: { clientId, pluginKey: 'npr' }, command: { name, payload: payload || {} }, mode: 'production' }) as Promise<NprCommandResponse>
}
...
export function searchAndWait(clientId: string, payload: any): Promise<NprResult> {
  return (async () => {
    ...
    return (result.data?.data || result.result || {}) as NprResult;
  })();
}
