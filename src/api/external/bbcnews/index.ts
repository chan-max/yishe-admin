...
export interface BbcnewsItem { id: string; title: string; url: string; [key: string]: any; }
export interface BbcnewsResult { success: boolean; query: string; count: number; total: number; items: BbcnewsItem[]; error?: string; }
export interface BbcnewsServiceStatus { connected?: boolean; available?: boolean; status?: string; state?: string; message?: string; supportedCommands?: string[]; }
export interface BbcnewsClientVO { clientId: string; isOnline?: boolean; [mod]?: BbcnewsServiceStatus | null; }
export interface BbcnewsCommandResponse { success: boolean; message: string; data?: any; }

function sendCmd(clientId: string, name: string, payload?: Record<string, any>) {
  return sendServiceCommand({ target: { clientId, pluginKey: 'bbcnews' }, command: { name, payload: payload || {} }, mode: 'production' }) as Promise<BbcnewsCommandResponse>
}
...
export function searchAndWait(clientId: string, payload: any): Promise<BbcnewsResult> {
  return (async () => {
    ...
    return (result.data?.data || result.result || {}) as BbcnewsResult;
  })();
}
