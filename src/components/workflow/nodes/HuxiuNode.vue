...
import { huxiuIcon } from "@/assets/icons/news";
...
<template>
  <div class="wf-node wf-node--huxiu">
    <Handle type="target" :position="Position.Top" />
    <div class="wf-node__header">
      <img :src="huxiuIcon" class="wf-node__icon" />
      <span class="wf-node__title">{{ data.label || "虎嗅" }}</span>
    </div>
...
.wf-node--huxiu .wf-node__header {
  display: flex;
  align-items: center;
  gap: 6px;
  margin-bottom: 4px;
}
.wf-node--huxiu .wf-node__icon {
  width: 16px;
  height: 16px;
  object-fit: contain;
}
.wf-node--huxiu .wf-node__title {
  font-weight: 600;
  color: var(--app-text-color);
}
.wf-node--huxiu .wf-node__subtitle {
  font-size: 11px;
  color: #ff4040;
  background: #ffecec;
  padding: 2px 6px;
