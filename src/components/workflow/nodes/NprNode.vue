...
import { nprIcon } from "@/assets/icons/news";
...
<template>
  <div class="wf-node wf-node--npr">
    <Handle type="target" :position="Position.Top" />
    <div class="wf-node__header">
      <img :src="nprIcon" class="wf-node__icon" />
      <span class="wf-node__title">{{ data.label || "NPR" }}</span>
    </div>
...
.wf-node--npr .wf-node__header {
  display: flex;
  align-items: center;
  gap: 6px;
  margin-bottom: 4px;
}
.wf-node--npr .wf-node__icon {
  width: 16px;
  height: 16px;
  object-fit: contain;
}
.wf-node--npr .wf-node__title {
  font-weight: 600;
  color: var(--app-text-color);
}
.wf-node--npr .wf-node__subtitle {
  font-size: 11px;
  color: #1b5e20;
  background: #e8f5e9;
  padding: 2px 6px;
