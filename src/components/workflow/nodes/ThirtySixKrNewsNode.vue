...
import { thirtySixKrNewsIcon } from "@/assets/icons/news";
...
<template>
  <div class="wf-node wf-node--36kr">
    <Handle type="target" :position="Position.Top" />
    <div class="wf-node__header">
      <img :src="thirtySixKrNewsIcon" class="wf-node__icon" />
      <span class="wf-node__title">{{ data.label || "36氪" }}</span>
    </div>
...
.wf-node--36kr .wf-node__header {
  display: flex;
  align-items: center;
  gap: 6px;
  margin-bottom: 4px;
}
.wf-node--36kr .wf-node__icon {
  width: 16px;
  height: 16px;
  object-fit: contain;
}
.wf-node--36kr .wf-node__title {
  font-weight: 600;
  color: var(--app-text-color);
}
.wf-node--36kr .wf-node__subtitle {
  font-size: 11px;
  color: #2574eb;
  background: #ebf2fd;
  padding: 2px 6px;
