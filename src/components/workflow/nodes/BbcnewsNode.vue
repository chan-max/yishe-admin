...
import { bbcnewsIcon } from "@/assets/icons/news";
...
<template>
  <div class="wf-node wf-node--bbcnews">
    <Handle type="target" :position="Position.Top" />
    <div class="wf-node__header">
      <img :src="bbcnewsIcon" class="wf-node__icon" />
      <span class="wf-node__title">{{ data.label || "BBC News" }}</span>
    </div>
...
.wf-node--bbcnews .wf-node__header {
  display: flex;
  align-items: center;
  gap: 6px;
  margin-bottom: 4px;
}
.wf-node--bbcnews .wf-node__icon {
  width: 16px;
  height: 16px;
  object-fit: contain;
}
.wf-node--bbcnews .wf-node__title {
  font-weight: 600;
  color: var(--app-text-color);
}
.wf-node--bbcnews .wf-node__subtitle {
  font-size: 11px;
  color: #bb1919;
  background: #fde8e8;
  padding: 2px 6px;
