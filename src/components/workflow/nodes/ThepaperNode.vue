...
import { thepaperIcon } from "@/assets/icons/news";
...
<template>
  <div class="wf-node wf-node--thepaper">
    <Handle type="target" :position="Position.Top" />
    <div class="wf-node__header">
      <img :src="thepaperIcon" class="wf-node__icon" />
      <span class="wf-node__title">{{ data.label || "澎湃新闻" }}</span>
    </div>
...
.wf-node--thepaper .wf-node__header {
  display: flex;
  align-items: center;
  gap: 6px;
  margin-bottom: 4px;
}
.wf-node--thepaper .wf-node__icon {
  width: 16px;
  height: 16px;
  object-fit: contain;
}
.wf-node--thepaper .wf-node__title {
  font-weight: 600;
  color: var(--app-text-color);
}
.wf-node--thepaper .wf-node__subtitle {
  font-size: 11px;
  color: #005bac;
  background: #e6f0f8;
  padding: 2px 6px;
...
.wf-node--thepaper .wf-node__type {
  font-size: 10px;
