...
import { pixabayIcon } from '@/assets/icons/apps'
...
<template>
  <div class="wf-node wf-node--pixabay">
    <Handle type="target" :position="Position.Top" />
    <div class="wf-node__header">
      <img :src="pixabayIcon" class="wf-node__icon" />
      <span class="wf-node__title">{{ data.label || 'Pixabay 采集' }}</span>
    </div>
    <div v-if="keyword" class="wf-node__subtitle">{{ keyword }}</div>
    <div v-if="maxCount" class="wf-node__badge">Limit: {{ maxCount }}</div>
    <div class="wf-node__type">免费图库</div>
