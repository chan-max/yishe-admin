...
import { gdeltIcon } from "@/assets/icons/news";
...
<template>
  <div class="wf-node wf-node--gdelt">
    <Handle type="target" :position="Position.Top" />
    <div class="wf-node__header">
      <img :src="gdeltIcon" class="wf-node__icon" />
      <span class="wf-node__title">{{ data.label || "GDELT" }}</span>
    </div>
...
.wf-node--gdelt .wf-node__header {
  display: flex;
  align-items: center;
  gap: 6px;
  margin-bottom: 4px;
}
.wf-node--gdelt .wf-node__icon {
  width: 16px;
  height: 16px;
  object-fit: contain;
}
.wf-node--gdelt .wf-node__title {
  font-weight: 600;
  color: var(--app-text-color);
}
.wf-node--gdelt .wf-node__subtitle {
  font-size: 11px;
  color: #1a73e8;
  background: #e8f0fe;
  padding: 2px 6px;
