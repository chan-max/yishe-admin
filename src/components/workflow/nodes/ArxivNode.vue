...
import { arxivIcon } from "@/assets/icons/news";
...
<template>
  <div class="wf-node wf-node--arxiv">
    <Handle type="target" :position="Position.Top" />
    <div class="wf-node__header">
      <img :src="arxivIcon" class="wf-node__icon" />
      <span class="wf-node__title">{{ data.label || "arXiv" }}</span>
    </div>
...
.wf-node--arxiv .wf-node__header {
  display: flex;
  align-items: center;
  gap: 6px;
  margin-bottom: 4px;
}
.wf-node--arxiv .wf-node__icon {
  width: 16px;
  height: 16px;
  object-fit: contain;
}
.wf-node--arxiv .wf-node__title {
  font-weight: 600;
  color: var(--app-text-color);
}
.wf-node--arxiv .wf-node__subtitle {
  font-size: 11px;
  color: #b31b1b;
  background: #fde8e8;
  padding: 2px 6px;
