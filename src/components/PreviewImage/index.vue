...
const handleXScroll = (index: number) => {
  const imgParentElement = document.querySelector(
    ".preview-footer-thumbs"
  ) as HTMLElement;
  const imgWrapElement = document.querySelector("#thumb-item-" + index) as HTMLElement;
  imgParentElement.scrollTo({
    left:
      imgWrapElement?.offsetLeft -
      (imgParentElement.offsetWidth - imgWrapElement.offsetWidth) / 2,
    behavior: "smooth",
  });
};
// 点击缩略图切换当前预览
    ...
    <div class="preview" @click="handleClickMask">
      <img
        ...
        :style="{
          top: imgTop + 'px',
          left: imgLeft + 'px',
          transform: `scale(${imgScale}) rotateZ(${imgRotate}deg)`,
          ...defaultOptions.previewStyle,
        }"
      ...
      <div class="preview-footer" @click.stop="preventDefault">
        <ul class="preview-footer-tools">
          <li class="icon-btn" v-if="imgList.length" @click="handleCut('last')">
            <img src="./assets/arrow-left.png" />
          </li>
          <li class="icon-btn" @click="handleRotate('left')">
            <img src="./assets/refresh-left.png" />
          </li>
          <li class="icon-btn" @click="handleScale('reduce')">
            <img src="./assets/reduce.png" />
          </li>
          <li class="icon-btn" @click="imgScale = 1">
            <img src="./assets/real-size.png" />
          </li>
          <li class="icon-btn" @click="handleScale('add')">
            <img src="./assets/add.png" />
          </li>
          <li class="icon-btn" @click="handleRotate('right')">
            <img src="./assets/refresh-right.png" />
          </li>
          <li class="icon-btn" v-if="imgList.length" @click="handleCut('next')">
        ...
        <template v-if="imgList.length">
          <div class="preview-footer-thumbs">
            <div
              ...
              class="thumb-item"
              :style="{
                background: currentIndex === index ? defaultOptions.activeColor : '',
              }"
          ...
          <div class="preview-footer-indexs">{{ getImageName(currentImg) }}</div>
          <div class="preview-footer-indexs">
      ...
      <span
        v-if="state.defaultOptions.showDownloadBtn"
        class="download-btn icon-btn"
        @click.stop="downloadFile(currentImg)"
      >
